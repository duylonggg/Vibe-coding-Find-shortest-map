 ...
 *   1. Order nodes by total degree (low degree first = low importance).
 *   2. Contract each node: for every remaining neighbour pair (u, w) not
 *      reachable by a shorter path avoiding v, add a shortcut edge u→w.
 *   3. Build directed upward graph: edge u→w only when level(w) > level(u).
 *
 * Query: bidirectional Dijkstra that relaxes only upward edges.
 *
 * The full query is pre-computed in start(); nextStep() replays the
 * exploration sequence one node at a time for the animation.
 */
class Ch extends PathfindingAlgorithm {
    constructor() {
        super();
        this.steps = [];      // [{node, referer}, ...]
        this.stepIndex = 0;
    }
    ...
    // ── Dijkstra step on upward/downward edges ────────────────────────────────
    _dijkstraStep(pq, visited, dist, parent, edgeMap, bestDist) {
        if (pq.length === 0) return null;
        pq.sort((a, b) => a.cost - b.cost);
        const { id: cur, cost } = pq.shift();
        if (visited.has(cur) || cost > bestDist) return null;
        visited.add(cur);
        for (const { to, weight } of edgeMap.get(cur) ?? []) {
    ...
    start(startNode, endNode) {
        ...
        // ── Collect all nodes (BFS from start) ───────────────────────────────
        ...
        // ── Node ordering: degree-based (low degree = low importance) ────────
        ...
        // ── Build upward graph (u→w where level(w) > level(u)) ───────────────
        ...
        const explorationF = []; // [{id, parentId}, ...]
        const explorationB = [];
        ...
        // Interleave forward and backward exploration steps
        ...
        if (meetId) {
            // Forward path: startNode … meetNode
            const fwdPath = [];
            let cur = meetId;
            while (cur !== null) {
            ...
            // Backward path: meetNode … endNode (parentB goes from endNode outward)
            const bwdPath = [];
            ...
            // Wire parent pointers along the full path for Map.jsx path tracing
